  ...
  for (const hostname of hostnames) {
    ...
    try {
      if (!zones.has(domainName)) {
        zone = await cloudflare.findZone(domainName); // Attempt to find the zone
        zones.set(domainName, zone);
      } else {
        zone = zones.get(domainName);
      }
    } catch (error) {
      console.log(`Error finding zone for domain ${domainName}:`, error);
      continue; // Skip this hostname if the zone is not found
    }

    ord = await cloudflare.findRecord(zone, hostname, isIPV4);

if (!record) {
  console.log(`DNS record not found for hostname: ${hostname}`);
  continue; // Skip to the next hostname if the record is not found
}

// If record is found, proceed to update
record.content = ip;
